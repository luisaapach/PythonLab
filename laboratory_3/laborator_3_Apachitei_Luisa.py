...
def function3(dict1, dict2):
    ...
    :return: (dict1-dict2) | (dict2-dict1) - e doar o comparatie intre valorile din dictionare 
    """
...
def build_xml_element(tag,content,**parameters):
    ...
    :return:
    """
    return "<" + tag + " " + r" ".join([p[0]+" = \""+ p[1] +"\"" if type(p[1])==str else p[0]+" = " + str(p[1]) for p in parameters.items()])+">"+content+"</"+tag+">"
