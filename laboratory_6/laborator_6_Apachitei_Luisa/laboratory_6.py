...
def function_3(text_chars,list_of_re):
    ...
    """
    return [el for el in text_chars if any(re.match(r,el) for r in list_of_re)]
...
def function_4(path_to_xml,attrs):
    ...
    the items selected will be those tags whose attributes are class="url" si name="url-form"
    si data-id="item".
    :param path_to_xml:
    :param attrs:
    ...
    with open(path_to_xml,"r") as f_d:
        for el in re.findall("<\w+.*?>",f_d.read()):
            if(all([re.search(item[0]+"\s*=\s*\""+item[1],el,flags=re.I)!=None for item in attrs.items()])):
                result+=[el]
    return result
...
def function_5(path_to_xml, attrs):
    ...
    with open(path_to_xml, "r") as f_d:
        for el in re.findall("<\w+.*?>", f_d.read()):
            if (any([re.search(item[0] + "\s*=\s*\"" + item[1], el, flags=re.I) != None for item in attrs.items()])):
                result += [el]
    return result
...
def function_7(string):
    return re.match(r"[12]\d\d(0[1-9]|1[0-2])(0[1-9]|[12]\d|3[01])\d{6}$",string) != None
